...
#FileName:promethues_api_level_2.py
#Function:
#Version:2.0
#Created:2023-04-07
#--------------------------------------------------import os
...
def prometheus_request(url, query):
    try:
        response = requests.get(url + 'query', params={'query': query})        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        print("请求错误：", e)
        return None

def process_result(result):    if result and result.get('data', {}).get('result'):
        return float(result['data']['result'][0]['value'][1])
    else:
        print("没有数据")
        return None

def write_log(path, ip, value):
    with open(path, 'a') as f:
        f.write(f"{ip} {value:.2f} {datetime.now().strftime('%Y-%m-%dT%H:%M:%S')}\n")
...
def main():
    ...
    ip_list = get_ip_list('/root/scripts/python3_8_9_project/iplist')
        ...
        for ip in ip_list:
            ...
            if current_time.minute % 15 == 0:
                ...
                if value_15m is not None:
                    write_log(f"{log_path}15.log", ip, value_15m)
            ...
            if current_time.minute % 5 == 0:
                ...
                if value_5m is not None:
                    write_log(f"{log_path}5.log", ip, value_5m)
            ...
            if value_1m is not None:
                write_log(f"{log_path}1.log", ip, value_1m)
